...
Lithium-ion batteries have become a principal energy-storage technology for electric vehicles, portable electronics, and grid-scale storage because of their high energy density, low self-discharge, and long service life. Their performance nevertheless deteriorates during operation through coupled aging processes, including solid electrolyte interphase (SEI) growth, lithium plating, and loss of active material. These processes reduce the available capacity and power capability and can increase operational risk \cite{han2019review,che2023health}.  To resolve this issue, accurate state-of-health (SOH) estimation is of critical importance for ensuring reliable and safe battery operation.

Early SOH-estimation studies relied mainly on electrochemical or equivalent-circuit models. Physics-based models, represented by the pseudo-two-dimensional model and its reduced single-particle variants, describe transport and reaction processes through coupled differential equations \cite{doyle1993modeling}. They provide physically interpretable states but require parameters that are difficult to identify and maintain as temperature, operating conditions, and aging modes change. Equivalent-circuit models approximate terminal behavior using voltage sources, resistors, and capacitors and are more suitable for real-time calculation \cite{hu2012comparative}. Their compact structure, however, cannot represent every internal aging mechanism, and estimation quality depends on the selected circuit topology and parameter-identification procedure.

The increasing availability of cycling data has encouraged data-driven SOH estimation \cite{li2019datadriven}. Conventional machine-learning methods first extract health indicators (HIs) from voltage, current, time, temperature, or derived electrochemical curves and then learn a mapping from these indicators to SOH. Gaussian process regression \cite{richardson2017gaussian} has been used both to forecast battery health and to quantify prediction uncertainty. Charging-curve features combined with an improved Gaussian process model can also provide accurate estimates without requiring a complete discharge trajectory \cite{yang2018novel}. Hu et al. \cite{hu2020battery} further compare multiple feature-selection strategies and regressors, showing that careful HI selection strongly affects both accuracy and computational efficiency. Although such models are compact and comparatively interpretable, their performance is tied to manually designed features. A feature set selected for one protocol or cell type may become unavailable or change its distribution under partial charging, different temperatures, or a new battery configuration.

Deep neural networks reduce some dependence on handcrafted mappings by learning degradation representations from cycling signals. Recurrent networks model the evolution of aging across cycles, whereas temporal convolution captures local patterns in voltage, current, and derived curves. Gated recurrent units and long short-term memory networks mitigate the optimization difficulties of ordinary recurrent networks, but recurrence still limits parallel computation and can weaken the transmission of information over very long sequences. Convolutional models are efficient at extracting local structure, although their effective receptive field depends on network depth, dilation, and kernel design. Hybrid architectures combine these complementary properties. For example, Lu et al. \cite{lu2023deep} use domain-adaptive deep learning to estimate SOH without additional degradation experiments for a target battery, while Xiong et al. \cite{xiong2024end} integrate multi-neighboring incomplete charging segments through a CNN-LSTM estimator. BiGRU-Transformer models \cite{jia2023state} have also been used to combine recurrent feature extraction with long-range dependency modeling. Despite these advances, task-specific networks are usually trained on limited labeled aging data, and their learned representations may shift across cells and operating protocols. Capacity regeneration creates an additional difficulty, cycle-level recovery can temporarily interrupt the overall fading trajectory and mislead a model that treats every upward change as sustained health improvement \cite{ma2021capacity}.

Inspired by its great advances in natural language processing (NLP) and computer vision (CV), Transformer has been introduced to battery SOH estimation. Gu et al. \cite{gu2023novel} combine convolutional feature extraction with a Transformer to model local and global patterns. Bai and Wang \cite{bai2023convolutional} aggregate multiview battery information with a convolutional Transformer and Chen et al.  \cite{chen2023vision} construct a vision-Transformer estimator from image-like battery representations. More recently, BMSFormer \cite{li2024bmsformer} introduces local-global fusion attention and depthwise separable convolutions to balance estimation accuracy and computational cost. Park et al. \cite{park2026configuration} use attention analysis to identify HIs that remain informative across cell and module configurations. These methods demonstrate the value of attention for SOH estimation, but most train their sequential representations specifically for a battery dataset. Limited aging samples can consequently restrict the transferable temporal prior that the model acquires.

Recently, Large Language Models (LLMs) have been the most promising method to process multiple NLP tasks, such as text generation, question answering, and machine translation. GPT-2 \cite{radford2019language} shows that Transformer language models trained on large corpora can transfer across tasks without changing the underlying architecture. GPT-3 \cite{brown2020language} subsequently demonstrates strong in-context performance after scaling autoregressive pretraining to 175 billion parameters. Their success has prompted research beyond natural language. GPT4TS \cite{zhou2023one} adapts frozen pretrained models to classification, forecasting, imputation, and anomaly detection by retaining their attention and feed-forward blocks while tuning a small set of parameters. PromptCast \cite{xue2023promptcast} instead represents numerical histories and targets as textual prompts for generative forecasting. However, directly applying existing strategies to battery data remains difficult. Multivariate HIs do not share the discrete semantics of words, their relationships are governed by coupled degradation processes, and transient recovery must be distinguished from the long-term aging trend. Therefore, a suitable battery adaptation should bridge the numerical-token modality gap while preserving both temporal structure and interactions among HIs.
