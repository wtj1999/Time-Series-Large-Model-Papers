...
Nowadays, with the accelerated deployment of electric vehicles, portable electronics, and grid-scale energy storage systems, lithium-ion batteries have become the dominant energy storage devices owing to their high energy density, low self-discharge rate, and long cycle life. However, during long-term operation, lithium-ion batteries inevitably suffer from complicated internal degradation mechanisms, such as solid electrolyte interphase (SEI) growth, lithium plating, and loss of active material, which gradually deteriorate their available capacity and may even trigger catastrophic safety accidents. To resolve this issue, accurate state-of-health (SOH) estimation is of critical importance for ensuring reliable and safe battery operation.

  Battery SOH estimation has been extensively investigated for decades. At the early stage, some model-based methods were developed to characterize the internal electrochemical phenomena of lithium-ion batteries. Among them, electrochemical models, such as the single-particle model and the pseudo-two-dimensional model [1], utilize a series of partial differential equations to describe the electrochemical reaction processes inside the battery. Although these models can offer high fidelity, they strongly rely on extensive parameter identification, which can be impractical for online estimation. Meanwhile, as another typical type of model-based approaches, equivalent circuit models (ECMs) [2,3] simulate the battery's internal behavior with circuit components such as resistors and capacitors. Although ECMs enjoy low computational complexity and easy parameter identification, they cannot fully capture the internal changes of batteries, often yielding degraded estimation accuracy.

  To overcome the limitations of model-based methods, machine learning (ML)-based methods have been extensively explored. These methods employ various algorithms to establish the relationships between health indicators (HIs) extracted from measured data and the battery's SOH. For instance, some methods [4–6] craft multiple HIs from voltage, current, and temperature curves, and feed them into Gaussian process regression (GPR), support vector machine (SVM), or random forest (RF) for SOH estimation. Although those methods are easy to interpret, they strongly rely on cumbersome procedures for HI selection and model identification, which is labor-intensive for a specific application. In addition, the ML-based approaches exhibit limited generalization capability because cycling data vary under different operating conditions and battery configurations, leading to discrepancies in features that ML-based models fail to effectively capture. As diverse end products necessitate different energy demands, the configurations of lithium-ion batteries vary, causing such cycling variations, and one has to train a SOH estimator separately for each configuration, resulting in substantial inefficiency.

  Over the years, benefiting from the prosperity of deep learning, deep neural network (DNN)-based methods have garnered significant attention. Due to their strong robustness of series modeling, DNN-based methods generally achieve superior performance than traditional ML ones. A class of DNN-based methods [7–9] introduce long short-term memory (LSTM) networks or gated recurrent units (GRU) to capture temporal dependencies in battery cycling series. The recurrent architecture can effectively model long-term degradation trends, but often suffers from the problem of gradient vanishing or explosion, significantly limiting the estimation performance. Another class of DNN-based methods apply the convolutional neural network (CNN) in the temporal domain to extract local degradation features. Due to the limitation of the one-dimension convolution kernels, they can only perceive bounded time slots, thus failing to extract long-term dependencies. Later, some methods [10–12] propose hybrid neural networks, which employ the CNN to extract local features and the LSTM to capture temporal dependencies, and can further improve the estimation accuracy. However, these DNN-based methods are typically trained from scratch on limited battery cycling data and cannot learn comprehensive sequential representations from large-scale cross-chemistry data, thus yielding undesirable generalization performance on unseen battery chemistries and configurations. Moreover, they struggle to handle the well-known random capacity regeneration phenomenon [13], easily mistaking transient capacity recovery for genuine health improvement.

  Inspired by its great advances in natural language processing (NLP) and computer vision (CV), Transformer has been introduced to battery SOH estimation. Benefiting from the attention mechanism, Transformer-based methods can effectively exploit long-term dependencies in cycling series data. For instance, some methods [14–16] propose the Transformer encoder–decoder structure for SOH estimation and achieve promising results. To improve computational efficiency, Li et al. [17] develop BMSFormer, which employs a Local-Global Fusion Attention module to reduce computational complexity while capturing both short-term and long-term features. To enable configuration-agnostic estimation, Park et al. [18] leverage the attention weights of a multi-objective transformer to identify five dominant HIs that are sensitive to SOH but insensitive to battery configuration. As existing Transformer-based methods are still limited in scale and trained from scratch, they may fail to fully unleash the potential
   of attention mechanisms in reasoning intricate degradation patterns obscured in entangled cycling series. The development of large-scale foundation models for battery health diagnostics remains nascent, which will be investigated in the present paper.

  Recently, Large Language Models (LLMs) have been the most promising method to process multiple NLP tasks, such as text generation, question answering, and machine translation. After being trained on large-scale text corpora, LLMs demonstrate the strong capability of learning intricate semantic representations and underlying relationships between entities. A case in point is the Generative Pre-trained Transformer (GPT), such as GPT-2 and GPT-3, which have billions of parameters and achieve impressive performance in few-shot and zero-shot learning. Recent advances of LLMs have explored their applications in time series data mining. Among them, GPT4TS [19] builds a unified framework with partially frozen LLMs for time series analysis, which only fine-tunes the positional embeddings and layer normalization layers while freezing the self-attention and feedforward layers of LLMs. Moreover, prompt tuning provides an efficient way to orient LLMs towards specific downstream tasks. Along this line, PromptCast [20] transforms the numerical input and output into prompts, aiming to directly apply LLMs for forecasting tasks. However, large models are rarely applied to battery SOH estimation. This is mainly due to the lack of a principled way to bridge the modality gap between multivariate cycling series and language tokens, as well as the absence of an effective scheme to incorporate battery domain knowledge into the pre-trained backbone. Existing LLM-based time series methods only leverage fixed or data-driven prompts. This naive manner may fail to capture the physically-meaningful health indicators and intrinsic degradation mechanisms, which are particularly important for battery SOH estimation.

 To exploit the powerful reasoning capability of LLMs for battery SOH estimation, we develop a novel Multi-Health-Indicator-enhanced Pre-Trained Large Language Model, namely MHILLM. We begin by constructing a comprehensive set of scalar health indicators from partial charging and discharging curves of each cycle, which includes constant-current charge time, constant-current discharge time, incremental-capacity peak magnitude and position, differential-voltage valley depth, sample entropy, and discharge voltage curve slope, among others. Then, to capture disentangled degradation patterns and mitigate the well-known random capacity regeneration phenomenon, we perform cycle-decomposition on each HI channel to separate its long-term fading trend from its short-term recovery component, and learn the disentangled patterns through individual modeling of the two components. To boost LLM's performance for battery health estimation, we innovatively devise a cross-HI attention module to explicitly model the pairwise interactions between HIs, enabling the model to adaptively emphasize the most degradation-informative indicators under varying operating conditions and suppress redundant ones. Moreover, to familiarize LLMs with battery HI sequences, we introduce a partial-freezing autoregressive strategy to fine-tune the backbone LLMs and learn HI-aware degradation representations for the downstream estimation task. Kindly note that the partial-freezing strategy only updates the positional embeddings and layer normalization layers, jointly with Low-Rank Adaptation (LoRA), retaining the powerful reasoning capability of pre-trained LLMs while adapting to the battery domain. The learned representations are subsequently fed into a lightweight regression layer to produce the final SOH estimate.
...
\item Extensive experiments on four public battery datasets demonstrate that our proposed MHILLM can significantly improve the estimation performance by well exploiting LLM's capability in the realm of battery SOH estimation, and exhibit strong cross-chemistry and cross-configuration generalization ability.
\end{itemize}
...
Early data-driven studies generally rely on handcrafted health indicators and conventional machine learning models. Richardson et al. employ Gaussian process regression (GPR) to forecast battery SOH and quantify the uncertainty associated with the prediction \cite{richardson2017gaussian}. Yang et al. extract four aging-sensitive features from charging curves and use gray relational analysis and an improved GPR model for SOH estimation \cite{yang2018novel}. Hu et al. further integrate fusion-based feature selection with machine learning to identify informative indicators from battery measurements \cite{hu2020battery}. These approaches achieve reliable estimation with relatively compact models. However, their accuracy is closely related to the quality of manually designed features, while the selected indicators may become unavailable or exhibit different distributions under partial charging, dynamic loads, and unseen battery chemistries.

Deep neural networks alleviate this dependence by automatically learning degradation representations from raw or lightly processed cycling signals. Convolutional neural networks capture local variations in voltage or incremental-capacity curves, whereas recurrent architectures model the evolution of degradation over successive cycles. Lu et al. introduce a deep learning framework that estimates SOH without requiring additional degradation experiments for the target battery, demonstrating the potential of knowledge transfer across cells \cite{lu2023deep}. Xiong et al. directly map multi-neighboring incomplete charging segments to SOH through an end-to-end CNN--LSTM framework \cite{xiong2024end}. Nevertheless, battery aging is reflected simultaneously by voltage, current, time, capacity, resistance, and their derived curves. A model built around a single indicator or a fixed charging interval may overlook complementary degradation information. Moreover, the limited amount of labeled aging data and the distribution shift among operating conditions still restrict the generalization of task-specific deep models. These limitations motivate the joint modeling of multiple health indicators and the reuse of knowledge learned through large-scale pretraining.

\subsection{Transformer for SOH}

The Transformer replaces recurrence with multi-head self-attention and can directly model pairwise dependencies over an entire sequence \cite{vaswani2017attention}. This property is particularly suitable for battery degradation signals, where informative changes may occur at distant sampling positions or across widely separated cycles. Compared with recurrent networks, Transformer-based estimators provide shorter information paths for long-term dependency modeling and allow different attention heads to characterize complementary degradation patterns.

Recent studies have adapted this architecture to SOH estimation from different battery representations. Gu et al. combine a convolutional network with a Transformer, using convolution to extract local patterns and self-attention to learn global dependencies from charging data \cite{gu2023novel}. Bai and Wang develop a convolutional Transformer-based multiview framework to aggregate degradation information from multiple views \cite{bai2023convolutional}. Chen et al. transform battery measurements into image-like representations and employ a vision Transformer for health estimation \cite{chen2023vision}. Jia et al. combine a bidirectional gated recurrent unit with a Transformer to model both sequential evolution and long-range relationships in capacity degradation \cite{jia2023state}. These studies demonstrate that attention mechanisms can effectively enhance global feature interaction beyond conventional CNN- or RNN-based estimators.

More recent work focuses on efficiency and practical measurement constraints. Li et al. propose BMSFormer, which uses a strongly correlated health indicator, depthwise feature fusion, and local--global fusion attention to balance estimation accuracy and computational cost \cite{li2024bmsformer}. Shu et al. select an informative partial voltage segment and use a Transformer to capture its temporal dependencies, enabling SOH estimation when complete charging curves are unavailable \cite{shu2025voltage}. Park et al. exploit attention weights to identify health indicators that are informative for SOH but insensitive to battery configuration, and construct a configuration-agnostic Transformer estimator \cite{park2026configuration}. Although these methods substantially advance long-range modeling, most Transformer-based SOH models are still trained from scratch for a predefined input representation and dataset. Consequently, their transferable knowledge is bounded by the scale and diversity of the available battery data. A pretrained backbone capable of adapting general sequential representations to multiple health indicators provides a promising direction for improving data efficiency and robustness.

\subsection{Pretrained Large Language Model}

Pretrained large language models (LLMs) learn general-purpose representations from large corpora and transfer the acquired knowledge to downstream tasks through prompting, fine-tuning, or representation reprogramming. Their Transformer backbones contain powerful sequence modeling modules, inspiring recent attempts to repurpose them for numerical time series. Instead of training a forecasting architecture from scratch, Zhou et al. retain the self-attention and feed-forward layers of a frozen pretrained model and adapt only a small subset of components, showing that pretrained language or vision Transformers can support forecasting, classification, anomaly detection, and few-shot time-series analysis \cite{zhou2023one}. Gruver et al. encode numerical observations as strings and formulate forecasting as next-token prediction, demonstrating the zero-shot forecasting capability of pretrained LLMs without task-specific parameter training \cite{gruver2023large}.

Another line of research aligns continuous time series with the representation space of an LLM. Jin et al. propose Time-LLM, which reprograms time-series patches using text prototypes and supplements them with a prompt-as-prefix while keeping the LLM backbone frozen \cite{jin2024timellm}. Chang et al. develop a two-stage fine-tuning strategy that first aligns a pretrained LLM with temporal patterns and then adapts it to downstream forecasting tasks \cite{chang2023llm4ts}. These methods indicate that the general sequence priors contained in pretrained LLMs can benefit numerical prediction, especially when labeled downstream data are scarce. However, direct reuse of an LLM does not automatically resolve the discrepancy between discrete language tokens and continuous battery measurements. Battery SOH estimation further differs from general forecasting because heterogeneous health indicators describe distinct but coupled electrochemical degradation phenomena and may have different scales, noise levels, and temporal resolutions.

Therefore, an effective LLM-based SOH estimator should not simply treat battery measurements as ordinary text tokens. It should construct an explicit interface between multiple health indicators and the pretrained representation space, while preserving indicator-specific information and enabling cross-indicator interaction. Existing Transformer-based battery studies mainly learn such interactions from limited battery data, whereas current LLM-based time-series methods are rarely tailored to battery health assessment. This gap motivates our multi-health-indicator-enhanced pretrained LLM, which leverages complementary degradation evidence to activate transferable sequential knowledge for accurate and robust SOH estimation.
...
\end{gather}
where $\boldsymbol{\mu}^{c},\boldsymbol{\sigma}^{c}\in\mathbb{R}^{N_h}$ are the mask-aware channel-wise mean and standard deviation calculated from observed entries along the cycle dimension for component $c\in\{\mathbf{T},\mathbf{R}\}$, $\boldsymbol{\gamma}^{c}$ and $\boldsymbol{\beta}^{c}$ are learnable affine parameters, and $\epsilon$ is a small constant for numerical stability. In implementation, the affine scaling is applied channel-wise through element-wise multiplication. The training-set z-score transformation in Section 3.2 first harmonizes the physical units of different HIs, whereas instance normalization further reduces battery-specific shifts within each input window. The two components are normalized independently because their amplitudes and temporal characteristics are substantially different. Nevertheless, the removed mean and variance may themselves indicate the absolute health level. We therefore preserve these statistics and encode them as degradation-context tokens in the subsequent tokenization stage. 
...
\textbf{Leakage-free autoregressive degradation alignment}.
