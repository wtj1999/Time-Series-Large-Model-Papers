...
\section{Conclusions}
In this paper, we introduce \textbf{MHILLM}, a novel framework that adapts a pretrained large language model to state-of-health estimation of lithium-ion batteries through multi-health-indicator modeling. Our approach addresses the difficulty of transferring a language-oriented sequential prior to continuous battery measurements by constructing battery tokens from multiple complementary HIs and explicitly organizing their degradation information before it is processed by the pretrained backbone. MHILLM first decomposes each HI sequence into trend and fluctuation components, allowing persistent capacity fading and short-term recovery behavior to be modeled in separate branches. It then employs cross-HI attention to capture the coupled relationships among heterogeneous indicators, while the context token preserves window-level degradation information that may be weakened during normalization. Another key design of MHILLM is the two-stage adaptation strategy. The autoregressive alignment stage learns to predict the subsequent HI vector and thereby aligns GPT-2 with battery degradation dynamics before SOH supervision is introduced. LoRA and partial parameter updating further adapt the pretrained representation to battery sequences without optimizing the complete backbone, after which a lightweight regression head produces the final one-step-ahead SOH estimate. Extensive cell-disjoint experiments on the NASA, CALCE, and Oxford datasets demonstrate that MHILLM consistently outperforms conventional regression, deep-learning, Transformer-based, and pretrained-model baselines across MAE, RMSE, and MAPE. The component ablations further verify the complementary contributions of sequence decomposition, cross-HI interaction, the context token, autoregressive alignment, and LoRA adaptation. Backbone comparisons show that GPT-2 provides a favorable balance between predictive accuracy and computational cost, while the parameter-sensitivity results demonstrate that moderate historical context, backbone depth, and attention granularity are sufficient for effective degradation modeling. These findings indicate that combining structured battery knowledge with the transferable sequential representation of a pretrained language model provides an effective solution for SOH estimation on unseen cells. Future work will investigate adaptation across a broader range of battery chemistries and operating conditions, uncertainty-aware SOH prediction, and model compression for deployment in resource-constrained battery-management systems.
